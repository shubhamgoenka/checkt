...
internal class SideMenuTransition: UIPercentDrivenInteractiveTransition, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate {
    ...
    open class func handleHideMenuTap(_ tap: UITapGestureRecognizer) {
        viewControllerForPresentedMenu?.dismiss(animated: true, completion: nil)
    }
    ...
    open class func hideMenuComplete() {
        let mvc = SideMenuTransition.viewControllerForPresentedMenu as? UINavigationController
        //print(mvc?.topViewController)
        mvc?.topViewController?.viewWillAppear(false)
